...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t(plugin?.manifestJson?.displayName ?? plugin?.packageName ?? "Plugin Details") },
    ]);
  ...
  const displayStatus = getStatusLabel(plugin.status, locale);
  const statusVariant =
    plugin.status === "ready"
      ? "default"
      : plugin.status === "error"
    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-4">
        <Link to="/instance/settings/plugins">
          <Button variant="outline" size="icon" className="h-8 w-8" aria-label={t("Back")}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
        </Link>
        <div className="flex items-center gap-2">
          <Puzzle className="h-6 w-6 text-muted-foreground" />
          <h1 className="text-xl font-semibold">{t(plugin.manifestJson.displayName ?? plugin.packageName)}</h1>
            ...
            <section className="space-y-5">
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  ...
                  <p className="text-sm leading-6 text-foreground/90">{t(pluginDescription)}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("Author")}</h3>
                    <p className="text-foreground">{t(plugin.manifestJson.author)}</p>
                  </div>
                  <div className="space-y-2">
                  ...
                  {dashboardData ? (
                    <>
                          ...
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              ...
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {getStatusLabel(dashboardData.worker.status, locale)}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              ...
                              <>
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    ...
                                    <span className="text-xs">{formatTimestamp(dashboardData.worker.lastCrashAt, locale)}</span>
                                  </div>
                                )}
                              </>
                            )}
                              ...
                              >
                                <div className="flex min-w-0 items-center gap-2">
                                  ...
                                  <Badge variant="outline" className="px-1 py-0 text-[10px]">
                                    {formatPluginTriggerLabel(run.trigger, t)}
                                  </Badge>
                                </div>
                                <div className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                                  {run.durationMs != null ? <span>{formatDuration(run.durationMs)}</span> : null}
                                  <span title={run.createdAt}>{formatRelativeTime(run.createdAt, locale)}</span>
                                </div>
                              </div>
                            ))}
                          </div>
                            ...
                            {dashboardData.recentWebhookDeliveries.map((delivery) => (
                              ...
                              >
                                ...
                                <div className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                                  ...
                                  <span title={delivery.createdAt}>{formatRelativeTime(delivery.createdAt, locale)}</span>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        ...
                        {t("Last checked: {time}", {
                          time: formatTimeValue(dashboardData.checkedAt, locale),
                        })}
                      </div>
                    </>
                  ) : (
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {t("Last {count} log entries", { count: recentLogs.length })}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="max-h-64 space-y-1 overflow-y-auto font-mono text-xs">
                      {recentLogs.map((entry) => (
                        ...
                        >
                          <span className="shrink-0 text-muted-foreground/50">{formatTimeValue(entry.createdAt, locale)}</span>
                    ...
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={healthData.healthy ? "default" : "destructive"}>
                          {getStatusLabel(healthData.status, locale)}
                        </Badge>
                      </div>
 ...
 */
function formatRelativeTime(isoString: string, locale: AppLocale): string {
  return formatRelativeTimeValue(isoString, locale);
}

/**
 * Format a unix timestamp (ms since epoch) to a locale string.
 */
function formatTimestamp(epochMs: number, locale: AppLocale): string {
  return formatDateTimeValue(new Date(epochMs), locale);
}

function formatPluginTriggerLabel(
  trigger: string,
  t: (text: string, values?: Record<string, string | number>) => string,
): string {
  if (trigger === "schedule") return t("Scheduled");
