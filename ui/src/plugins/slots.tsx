 ...
 *
 * Plugin bundles are built with `external: ["@swarmifyx/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__swarmifyxPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__swarmifyxPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__swarmifyxPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__swarmifyxPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@swarmifyx/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@swarmifyx/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@swarmifyx/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@swarmifyx/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@swarmifyx/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__swarmifyxPluginBridge__) {
 ...
 * self-register via `window.swarmifyxPlugins.registerReactComponent()`.
 ...
 * of `@swarmifyx/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
