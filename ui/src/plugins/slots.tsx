 ...
 *
 * Plugin bundles are built with `external: ["@papertape/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__papertapePluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__papertapePluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__papertapePluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__papertapePluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@papertape/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@papertape/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@papertape/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@papertape/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@papertape/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__papertapePluginBridge__) {
 ...
 * self-register via `window.papertapePlugins.registerReactComponent()`.
 ...
 * of `@papertape/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
