...
const JWT_SECRET_ENV_KEY = "PAPERTAPE_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Papertape environment variables",
    "# Generated by Papertape CLI commands",
...
export function resolvePapertapeEnvFile(configPath?: string): string {
...
export function loadPapertapeEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergePapertapeEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readPapertapeEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writePapertapeEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergePapertapeEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readPapertapeEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writePapertapeEnvEntries(next, filePath);
  return next;
}
