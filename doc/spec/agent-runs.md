...
This spec defines how Papertape actually runs agents while staying runtime-agnostic.
...
1. Papertape is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Papertape manages lifecycle, and on exit Papertape parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `papertape.skill` (shared Papertape skill text block)
...
2. Preferred transport is env vars (`PAPERTAPE_*`) injected at runtime.
