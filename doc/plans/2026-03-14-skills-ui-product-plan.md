...
This document defines the product and UI plan for skill management in Abacus.
...
- Abacus company import/export can include skills as package content
...
3. The UI must always tell the truth about what Abacus knows:
   - desired state in Abacus
...
Abacus should treat skills at two scopes:
...
- created locally inside Abacus later
...
7. “Understand what Abacus knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Abacus-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Abacus did not install.

These should be clearly marked:

- external
- not managed by Abacus
...
- `desired_only`: Abacus wants it, adapter does not show it yet
- `external`: adapter has it but Abacus does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Abacus only tracks desired state
...
- Abacus can still track desired skills
...
- `.abacus.yaml` may add Abacus-specific fidelity, but should not replace the base shortname association model
...
- `.abacus.yaml` may add Abacus-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Abacus should have a company-scoped skill model or managed package model representing:
...
Abacus should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Abacus-specific.
...
That gives Abacus one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
