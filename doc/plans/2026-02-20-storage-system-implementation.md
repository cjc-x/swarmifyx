...
Add a single storage subsystem for Papertape that supports:
...
- API serves attachment bytes through authenticated Papertape endpoints.
...
- Default local path is under instance root: `~/.papertape/instances/<instanceId>/data/storage`.
...
- `papertape onboard` writes a valid `storage` config block by default.
- `papertape configure --section storage` can switch between local and s3 modes.
...
- `papertape doctor` reports actionable storage status.
