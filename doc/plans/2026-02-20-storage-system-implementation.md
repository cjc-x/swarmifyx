...
- `swarmifyxai onboard` writes a valid `storage` config block by default.
- `swarmifyxai configure --section storage` can switch between local and s3 modes.
...
- `swarmifyxai doctor` reports actionable storage status.
