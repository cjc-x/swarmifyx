...
```sh
pnpm swarmifyxai configure --section server
```
...
1. `pnpm swarmifyxai onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm swarmifyxai doctor` works flagless with mode-aware checks.
