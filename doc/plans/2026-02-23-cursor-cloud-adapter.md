...
This document defines the V1 design for a Swarmifyx adapter that integrates with
...
4. synthesized stdout events for Swarmifyx UI/CLI
...
3. **Agent callback auth to Swarmifyx** uses a bootstrap exchange flow (no long-lived Swarmifyx key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Swarmifyx endpoints, not full SKILL.md prompt injection.
...
- `swarmifyxPublicUrl` (optional override; else `SWARMIFYX_PUBLIC_URL` env)
...
## Swarmifyx Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `SWARMIFYX_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.swarmifyxPublicUrl`
2. `process.env.SWARMIFYX_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Swarmifyx credentials in prompt text.

Flow:

1. Before launch/follow-up, Swarmifyx mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Swarmifyx validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Swarmifyx API calls.
...
1. Prompt includes a compact instruction to fetch skills from Swarmifyx.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/swarmifyx`
   - `GET /api/skills/swarmifyx-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <swarmifyxPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Swarmifyx run context.
...
5. `swarmifyxPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Swarmifyx skill endpoints |
| Swarmifyx auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Swarmifyx context.
3. Add richer PR metadata surfacing in Swarmifyx UI.
