...
This document defines the V1 design for a Papertape adapter that integrates with
...
4. synthesized stdout events for Papertape UI/CLI
...
3. **Agent callback auth to Papertape** uses a bootstrap exchange flow (no long-lived Papertape key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Papertape endpoints, not full SKILL.md prompt injection.
...
- `papertapePublicUrl` (optional override; else `PAPERTAPE_PUBLIC_URL` env)
...
## Papertape Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `PAPERTAPE_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.papertapePublicUrl`
2. `process.env.PAPERTAPE_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Papertape credentials in prompt text.

Flow:

1. Before launch/follow-up, Papertape mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Papertape validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Papertape API calls.
...
1. Prompt includes a compact instruction to fetch skills from Papertape.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/papertape`
   - `GET /api/skills/papertape-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <papertapePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Papertape run context.
...
5. `papertapePublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Papertape skill endpoints |
| Papertape auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Papertape context.
3. Add richer PR metadata surfacing in Papertape UI.
