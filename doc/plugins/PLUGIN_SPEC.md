...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@abacus-lab/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@abacus-lab/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@abacus-lab/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@abacus-lab/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@abacus-lab/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@abacus-lab/plugin-sdk/ui`
...
The `@abacus-lab/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
### 27.1 `@abacus-lab/plugin-test-harness`
...
```ts
import { createTestHarness } from "@abacus-lab/plugin-test-harness";
...
- `@abacus-lab/plugin-workspace-files`
- `@abacus-lab/plugin-terminal`
- `@abacus-lab/plugin-git`
- `@abacus-lab/plugin-linear`
- `@abacus-lab/plugin-github-issues`
- `@abacus-lab/plugin-grafana`
- `@abacus-lab/plugin-runtime-processes`
- `@abacus-lab/plugin-stripe`
...
- `@abacus-lab/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@abacus-lab/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@abacus-lab/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@abacus-lab/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@abacus-lab/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@abacus-lab/plugin-sdk/ui`
...
- `@abacus-lab/plugin-test-harness`
