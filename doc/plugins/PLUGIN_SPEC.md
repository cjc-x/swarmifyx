# Papertape Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Papertape's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Papertape app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Papertape HTTP APIs directly.
...
Papertape plugin design is based on the following assumptions:

1. Papertape is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Papertape business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Papertape core.
...
2. Let plugins add major capabilities without editing Papertape core.
...
4. Depend on project-local plugin folders such as `.papertape/plugins`.
...
The single Papertape deployment an operator installs and controls.

### 5.2 Company

A first-class Papertape business object inside the instance.
...
A trusted in-process extension loaded directly by Papertape core.
...
An installable instance-wide extension package loaded through the Papertape plugin runtime.
...
Papertape has two extension classes.
...
Papertape already has a concrete workspace model:
...
Plugins live under the Papertape instance directory.

Suggested layout:

- `~/.papertape/instances/default/plugins/package.json`
- `~/.papertape/instances/default/plugins/node_modules/`
- `~/.papertape/instances/default/plugins/.cache/`
- `~/.papertape/instances/default/data/plugins/<plugin-id>/`
...
Papertape should add CLI commands:

- `pnpm papertape plugin list`
- `pnpm papertape plugin install <package[@version]>`
- `pnpm papertape plugin uninstall <plugin-id>`
- `pnpm papertape plugin upgrade <plugin-id> [version]`
- `pnpm papertape plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@papertape/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface PapertapePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumPapertapeVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Papertape agents can use during runs.
...
- Papertape server starts one worker process per installed plugin
...
Receives one typed Papertape domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@papertape/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@papertape/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@papertape/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@papertape/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@papertape/plugin-sdk/ui`
...
The `@papertape/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Papertape secret provider system rather than a plain text input
...
1. Core Papertape data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Papertape objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Papertape product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Papertape later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Papertape secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm papertape plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Papertape server. This is a normative requirement, not optional.
...
### 27.1 `@papertape/plugin-test-harness`
...
```ts
import { createTestHarness } from "@papertape/plugin-test-harness";
...
For developing a plugin against a running Papertape instance:

- The operator installs the plugin from a local path: `pnpm papertape plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-papertape-plugin`) that scaffolds:

- `package.json` with correct `papertapePlugin` keys
...
- `@papertape/plugin-workspace-files`
- `@papertape/plugin-terminal`
- `@papertape/plugin-git`
- `@papertape/plugin-linear`
- `@papertape/plugin-github-issues`
- `@papertape/plugin-grafana`
- `@papertape/plugin-runtime-processes`
- `@papertape/plugin-stripe`
...
- `@papertape/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@papertape/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@papertape/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@papertape/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@papertape/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@papertape/plugin-sdk/ui`
...
- `@papertape/plugin-test-harness`
- `create-papertape-plugin` starter template
...
Papertape should not implement a generic in-process hook bag modeled directly after local coding tools.

Papertape should implement:
...
That is the complete target design for the Papertape plugin system.
