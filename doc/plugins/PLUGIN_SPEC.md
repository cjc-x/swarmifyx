...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Swarmifyx app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Swarmifyx HTTP APIs directly.
