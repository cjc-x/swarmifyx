...
This guide describes the current, implemented way to create a Swarmifyx plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Swarmifyx app.
...
Inside this monorepo, the scaffold uses `workspace:*` for `@swarmifyx/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@swarmifyx/plugin-sdk` from the local Swarmifyx checkout into a `.swarmifyx-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Swarmifyx from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@swarmifyx/plugin-sdk/ui`
