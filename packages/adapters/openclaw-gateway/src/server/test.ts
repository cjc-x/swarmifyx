...
import { asString, parseObject } from "@papertape/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Papertape server host.",
        });
      }
    } catch (err) {
      checks.push({
