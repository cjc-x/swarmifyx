...
This document describes how `@papertape/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Papertape logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Papertape `runId`)
