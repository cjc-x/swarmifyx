...
Example Papertape plugin that demonstrates:
...
```bash
pnpm --filter @papertape/plugin-file-browser-example build
pnpm papertape plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm papertape plugin uninstall papertape-file-browser-example --force
```
...
- Optional: use `papertape-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
