...
Example Swarmifyx plugin that demonstrates:
...
```bash
pnpm --filter @swarmifyx/plugin-file-browser-example build
pnpm swarmifyx plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm swarmifyx plugin uninstall swarmifyx-file-browser-example --force
```
...
- Optional: use `swarmifyx-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
