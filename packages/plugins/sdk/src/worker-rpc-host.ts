...
import type { SwarmifyxPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@swarmifyx/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: SwarmifyxPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@swarmifyx/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
