...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Abacus.
...
- Abacus must not require extra top-level fields for skill validity
- Abacus-specific extensions must live under `metadata.abacus` or `metadata.sources`
...
For Abacus, the preferred fidelity extension is:
...
- issue/task Abacus-only metadata
...
- Abacus tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Abacus shape:
...
Additional rules for Abacus exporters:
...
- omit empty and default-valued Abacus fields when possible
...
- Abacus exporters should emit `.abacus.yaml` as a sidecar by default
...
Abacus can map this spec to its runtime model like this:
...
- Abacus extension:
  - `.abacus.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, and other Abacus-specific fidelity

Inline Abacus-only metadata that must live inside a shared markdown file should use:

- `metadata.abacus`

That keeps the base format broader than Abacus.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Abacus.

## 20. Cutover

Abacus should cut over to this markdown-first package model as the primary portability format.

`abacus.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Abacus, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
