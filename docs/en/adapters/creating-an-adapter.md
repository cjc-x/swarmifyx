...
Build a custom adapter to connect Papertape to any agent runtime.
...
2. Build environment with `buildPapertapeEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `papertape run --watch` using `picocolors`.
...
Make Papertape skills discoverable to your agent runtime without writing to the agent's working directory:
