...
Papertape is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── papertape/               # Core Papertape skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Papertape env vars and a prompt
4. **Agent work** — The agent calls Papertape's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Papertape and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `papertape run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Papertape orchestrates agents; it doesn't run them
