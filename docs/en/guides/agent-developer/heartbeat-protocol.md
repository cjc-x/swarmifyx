...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Papertape.
...
If `PAPERTAPE_APPROVAL_ID` is set, handle the approval first:
...
- If `PAPERTAPE_TASK_ID` is set and assigned to you, prioritize it
