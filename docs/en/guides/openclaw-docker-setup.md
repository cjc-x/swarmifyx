...
How to get OpenClaw running in a Docker container for local development and testing the Papertape OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Papertape includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-papertape-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Papertape host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-papertape-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `PAPERTAPE_HOST_PORT` (default `3100`)
- `PAPERTAPE_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Papertape deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Papertape server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Papertape URL (typically `http://host.docker.internal:3100`).
- If Papertape rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm papertape allowed-hostname host.docker.internal
```

Then restart Papertape and rerun the smoke script.
